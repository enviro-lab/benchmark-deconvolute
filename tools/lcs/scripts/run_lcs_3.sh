...
### NOTES FOR RERUNNING WITH OTHER PLATES (we copied over software/LCS to software/LCS2 for our second plate and so on and then did this step):
## Files to (re)move first to ensure the whole pipeline is run anew for each plate:
# outputs/pool_map
# outputs/pool_mutect
...
### USAGE:
# sbatch --mail-user=$USER@uncc.edu --mail-type=END,FAIL /projects/enviro_lab/decon_compare/lcs/scripts/run_lcs_3.sh
