...
# NOTE: Based on https://github.com/baymlab/wastewater_analysis#building-a-reference-set
...
def main():
    ...
    parser.add_argument('--state', dest='state', nargs="*", help="only consider sequences found in specified state(s)")
    ...
    for lin_id in lineages:
        ...
        if args.state:
            samples = samples.loc[samples["state"].isin(args.state)]
        if args.startdate:
            samples = samples.loc[
    ...
    # This step allows the original script to be split into two parts: metadata selection & fasta writing
    # save list of sample accessions to download and details for writing fastas
    print("Saving sample accessions and lineage info...")
    with open(f"{args.outdir}/epicov_accessions.csv",'w') as out:
        for lin_id, gisaid_id in selection_dict.values():
            out.write(gisaid_id+"\n")
    with open(f"{args.outdir}/selection_dict.json",'w') as out:
        out.write(json.dumps(selection_dict))
...
def read_metadata(metadata_file, max_N_content, min_seq_len):
    ...
    if metadata_file.endswith(".tar.xz"):
        import tarfile
        with tarfile.open(metadata_file) as tar:
            df = pd.read_csv(tar.extractfile("metadata.tsv"),header=0,sep="\t")
    else:
